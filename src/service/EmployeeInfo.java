...
public class EmployeeInfo {
    ...
    public void addEmp(Employee emp){
     ...
     public String findSameEmp() {
         for (int i = 0; i < (Employee.countEmp - 1); i++) {
             for (int j = (i + 1); j < Employee.countEmp; j++) {
                 if (employee[i].equals(employee[j])) {
                     sameEmp = employee[i].getName() + " is entered in rows "+ i + " and "+j;
                     System.out.println(sameEmp);
                     sameEmpCount++;
                 }
             }
         }return sameEmpCount+ " employee(s) entered more than one time";
